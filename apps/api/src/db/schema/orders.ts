...
export const order = pgTable(
  ...
  (table) => [
    ...
    check("orders_totals_nonnegative", sql`${table.subtotal} >= 0 AND ${table.shippingTotal} >= 0 AND ${table.taxTotal} >= 0 AND ${table.discountTotal} >= 0 AND ${table.total} >= 0`),
  ],
);
...
export const orderItem = pgTable(
  ...
  (table) => [
    ...
    check("order_items_prices_nonnegative", sql`${table.unitPrice} >= 0 AND ${table.lineTotal} >= 0`),
  ],
);

export const orderRelations = relations(order, ({ many, one }) => ({
  user: one(user, { fields: [order.userId], references: [user.id] }),
  shippingAddressRecord: one(address, { fields: [order.shippingAddressId], references: [address.id] }),
