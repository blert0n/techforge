...
export const createProduct: RouteHandler<typeof createProductRoute> = async (
  c,
) => {
  ...
  return c.json(
    {
      ...
      discountPrice:
        savedProduct.discountPrice === null
          ? null
          : Number(savedProduct.discountPrice),
      stock: savedProduct.stock,
      status: savedProduct.status as "draft" | "active",
      ...
      attributeKeys: savedProduct.attributes.map((attribute) =>
        attribute.attributeName.replace(
          `${savedProduct.category.attributePrefix}.`,
          "",
        ),
      ),
    },
...
export async function updateAdminProduct(c: Context) {
  ...
  const updated = await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(eq(productSpecification.productId, id));
...
export const deleteProduct: RouteHandler<typeof deleteProductRoute> = async (
  c,
) => {
